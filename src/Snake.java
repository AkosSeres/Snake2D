...
public class Snake{
	...
	public boolean move(final int sizeX, final int sizeY){
		...
		switch(state){
		case GO_UP:
			if((pieces.get(0).y - 1) == -1)return false;
			pieces.get(0).y--;
			break;
		case GO_DOWN:
			if((pieces.get(0).y + 1) == sizeY)return false;
			pieces.get(0).y++;
			break;
		case GO_LEFT:
			if((pieces.get(0).x - 1) == -1)return false;
			pieces.get(0).x--;
			break;
		case GO_RIGHT:
			if((pieces.get(0).x + 1) == sizeX)return false;
			pieces.get(0).x++;
			break;
		}
		...
		for (int i = 1;i<pieces.size();i++){
			if((pieces.get(0).x == pieces.get(i).x) && (pieces.get(0).y == pieces.get(i).y))return false;
		}
